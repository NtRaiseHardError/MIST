...
Quicklz::~Quicklz() {
	if(this->state_compress)
		delete(this->state_compress);

	if (this->state_decompress)
		delete(this->state_decompress);
}

size_t Quicklz::GetCompressedSize(unsigned char *input) {
	return qlz_size_compressed(reinterpret_cast<char *>(input));
}

size_t Quicklz::GetDecompressedSize(unsigned char *input) {
	return qlz_size_decompressed(reinterpret_cast<char *>(input));
}

void Quicklz::Compress(unsigned char *output, const unsigned char *input, size_t &outlen, size_t inlen) {
	this->state_compress = new qlz_state_compress;

	outlen = qlz_compress(input, reinterpret_cast<char *>(output), inlen, this->state_compress);
}

void Quicklz::Decompress(unsigned char *output, unsigned char *input, size_t &outlen) {
	this->state_decompress = new qlz_state_decompress;
	
	outlen = qlz_decompress(reinterpret_cast<char *>(input), output, this->state_decompress);
}
